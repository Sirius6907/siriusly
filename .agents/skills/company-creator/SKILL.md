...
description: >
  ...
  is already running in SiriusEcoSystem.
---
...
└── .sirius-eco-system.yaml            (SiriusEcoSystem vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .sirius-eco-system.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [SiriusEcoSystem](https://github.com/sirius-eco-system/sirius-eco-system)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to SiriusEcoSystem (https://github.com/sirius-eco-system/sirius-eco-system)
- A "Getting Started" section explaining how to import: `sirius-eco-system company import --from <path>`
...
## .sirius-eco-system.yaml Guidelines

The `.sirius-eco-system.yaml` file is the SiriusEcoSystem vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — SiriusEcoSystem will use its default. Specifying an unknown adapter type causes an import error.

SiriusEcoSystem's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.sirius-eco-system.yaml`.
