/**
 * esbuild configuration for building the sirius-eco-system CLI for npm.
 *
 * Bundles all workspace packages (@sirius-eco-system/*) into a single file.
...
// to npm and resolved at runtime (e.g. @sirius-eco-system/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@sirius-eco-system/")) {
      externals.add(name);
    }
  }
