...
import { ApiConnectionError, ApiRequestError, SiriusEcoSystemApiClient } from "../client/http.js";

describe("SiriusEcoSystemApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new SiriusEcoSystemApiClient({
    ...
    expect(headers["x-sirius-eco-system-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new SiriusEcoSystemApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm sirius-eco-system run/,
    );
  });
