...
function buildSourceConfig(): SiriusEcoSystemConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("sirius-eco-system-pr-432")).toBe(
      path.resolve(os.homedir(), "sirius-eco-system-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("siriusEcoSystem/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://siriusEcoSystem.example", 3110)).toBe("https://siriusEcoSystem.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/sirius-eco-system-feature",
      homeDir: "/tmp/sirius-eco-system-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/sirius-eco-system-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/sirius-eco-system-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.SIRIUSLY_HOME).toBe(path.resolve("/tmp/sirius-eco-system-worktrees"));
    expect(env.SIRIUSLY_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.SIRIUSLY_IN_WORKTREE).toBe("true");
    expect(env.SIRIUSLY_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.SIRIUSLY_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export SIRIUSLY_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-secrets-"));
    const originalInlineMasterKey = process.env.SIRIUSLY_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.SIRIUSLY_SECRETS_MASTER_KEY;
      delete process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.SIRIUSLY_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          SIRIUSLY_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.SIRIUSLY_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.SIRIUSLY_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".sirius-eco-system-worktrees"),
      });

      const envPath = path.join(repoRoot, ".siriusEcoSystem", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("SIRIUSLY_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("SIRIUSLY_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/SIRIUSLY_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.SIRIUSLY_AGENT_JWT_SECRET;
      } else {
        process.env.SIRIUSLY_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".sirius-eco-system-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".siriusEcoSystem", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local SiriusEcoSystem config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".siriusEcoSystem", "config.json");
    const originalCwd = process.cwd();
    const originalSiriusEcoSystemConfig = process.env.SIRIUSLY_CONFIG;

    try {
      fs.mkdirSync(path.dirname(localConfigPath), { recursive: true });
      fs.writeFileSync(localConfigPath, JSON.stringify(buildSourceConfig()), "utf8");
      delete process.env.SIRIUSLY_CONFIG;
      process.chdir(repoRoot);

      expect(fs.realpathSync(resolveSourceConfigPath({}))).toBe(fs.realpathSync(localConfigPath));
    } finally {
      process.chdir(originalCwd);
      if (originalSiriusEcoSystemConfig === undefined) {
        delete process.env.SIRIUSLY_CONFIG;
      } else {
        process.env.SIRIUSLY_CONFIG = originalSiriusEcoSystemConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/siriusEcoSystem",
        targetRepoRoot: "/Users/example/sirius-eco-system-pr-432",
        workspaceCwd: "/Users/example/siriusEcoSystem",
      }),
    ).toBe("/Users/example/sirius-eco-system-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/siriusEcoSystem",
        targetRepoRoot: "/Users/example/sirius-eco-system-pr-432",
        workspaceCwd: "/Users/example/siriusEcoSystem/packages/db",
      }),
    ).toBe("/Users/example/sirius-eco-system-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sirius-eco-system-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "sirius-eco-system-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("sirius-eco-system-make-test", {
        seed: false,
        home: path.join(tempRoot, ".sirius-eco-system-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".siriusEcoSystem", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".siriusEcoSystem", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
