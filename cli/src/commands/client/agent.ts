...
import type { Agent } from "@sirius-eco-system/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveSiriusEcoSystemSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export SIRIUSLY_API_URL='${escaped(input.apiBase)}'`,
    `export SIRIUSLY_COMPANY_ID='${escaped(input.companyId)}'`,
    `export SIRIUSLY_AGENT_ID='${escaped(input.agentId)}'`,
    `export SIRIUSLY_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local SiriusEcoSystem skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing SiriusEcoSystem skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveSiriusEcoSystemSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local SiriusEcoSystem skills directory. Expected ./skills in the repo checkout.",
              );
            }
