...
import { ApiRequestError, SiriusEcoSystemApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to SiriusEcoSystem config file")
    .option("-d, --data-dir <path>", "SiriusEcoSystem data directory root (isolates state from ~/.siriusEcoSystem)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the SiriusEcoSystem API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SIRIUSLY_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.SIRIUSLY_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.SIRIUSLY_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SIRIUSLY_COMPANY_ID, or set context profile companyId via `sirius-eco-system context set`.",
    );
  }

  const api = new SiriusEcoSystemApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SIRIUSLY_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SIRIUSLY_SERVER_PORT || "");
