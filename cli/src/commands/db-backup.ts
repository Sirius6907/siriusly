...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@sirius-eco-system/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://siriusEcoSystem:siriusEcoSystem@127.0.0.1:${port}/siriusEcoSystem`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" siriusEcoSystem db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveSiriusEcoSystemInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "siriusEcoSystem";
