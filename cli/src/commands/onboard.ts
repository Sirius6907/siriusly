...
type OnboardDefaults = Pick<SiriusEcoSystemConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "SIRIUSLY_PUBLIC_URL",
  "DATABASE_URL",
  "SIRIUSLY_DB_BACKUP_ENABLED",
  "SIRIUSLY_DB_BACKUP_INTERVAL_MINUTES",
  "SIRIUSLY_DB_BACKUP_RETENTION_DAYS",
  "SIRIUSLY_DB_BACKUP_DIR",
  "SIRIUSLY_DEPLOYMENT_MODE",
  "SIRIUSLY_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "SIRIUSLY_ALLOWED_HOSTNAMES",
  "SIRIUSLY_AUTH_BASE_URL_MODE",
  "SIRIUSLY_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "SIRIUSLY_STORAGE_PROVIDER",
  "SIRIUSLY_STORAGE_LOCAL_DIR",
  "SIRIUSLY_STORAGE_S3_BUCKET",
  "SIRIUSLY_STORAGE_S3_REGION",
  "SIRIUSLY_STORAGE_S3_ENDPOINT",
  "SIRIUSLY_STORAGE_S3_PREFIX",
  "SIRIUSLY_STORAGE_S3_FORCE_PATH_STYLE",
  "SIRIUSLY_SECRETS_PROVIDER",
  "SIRIUSLY_SECRETS_STRICT_MODE",
  "SIRIUSLY_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveSiriusEcoSystemInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.SIRIUSLY_PUBLIC_URL?.trim() ||
    process.env.SIRIUSLY_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.SIRIUSLY_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.SIRIUSLY_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.SIRIUSLY_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.SIRIUSLY_ALLOWED_HOSTNAMES
    ? process.env.SIRIUSLY_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.SIRIUSLY_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.SIRIUSLY_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.SIRIUSLY_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.SIRIUSLY_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.SIRIUSLY_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.SIRIUSLY_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.SIRIUSLY_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.SIRIUSLY_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.SIRIUSLY_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.SIRIUSLY_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.SIRIUSLY_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.SIRIUSLY_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.SIRIUSLY_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.SIRIUSLY_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "SIRIUSLY_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<SiriusEcoSystemConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printSiriusEcoSystemCliBanner();
  p.intro(pc.bgCyan(pc.black(" sirius-eco-system onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveSiriusEcoSystemInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: SiriusEcoSystemConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@sirius-eco-system/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `sirius-eco-system doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("SIRIUSLY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.SIRIUSLY_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("SIRIUSLY_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("SIRIUSLY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: SiriusEcoSystemConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: SIRIUSLY_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("sirius-eco-system run")}`,
      `Reconfigure later: ${pc.cyan("sirius-eco-system configure")}`,
      `Diagnose setup: ${pc.cyan("sirius-eco-system doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
    const answer = await p.confirm({
      message: "Start SiriusEcoSystem now?",
      initialValue: true,
    });
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.SIRIUSLY_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("sirius-eco-system run")}`,
        `Then: ${pc.cyan("sirius-eco-system auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
