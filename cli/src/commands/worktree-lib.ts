...
export const DEFAULT_WORKTREE_HOME = "~/.sirius-eco-system-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".siriusEcoSystem");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: SiriusEcoSystemConfig | null;
  paths: WorktreeLocalPaths;
  serverPort: number;
  databasePort: number;
  now?: Date;
}): SiriusEcoSystemConfig {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "siriusEcoSystem",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    SIRIUSLY_HOME: paths.homeDir,
    SIRIUSLY_INSTANCE_ID: paths.instanceId,
    SIRIUSLY_CONFIG: paths.configPath,
    SIRIUSLY_CONTEXT: paths.contextPath,
    SIRIUSLY_IN_WORKTREE: "true",
    ...(branding?.name ? { SIRIUSLY_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { SIRIUSLY_WORKTREE_COLOR: branding.color } : {}),
  };
}
