...
} from "@sirius-eco-system/db";
import type { Command } from "commander";
import { ensureAgentJwtSecret, loadSiriusEcoSystemEnvFile, mergeSiriusEcoSystemEnvEntries, readSiriusEcoSystemEnvEntries, resolveSiriusEcoSystemEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { SiriusEcoSystemConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printSiriusEcoSystemCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.SIRIUSLY_CONFIG;
...
const WORKTREE_NAME_PREFIX = "sirius-eco-system-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.SIRIUSLY_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.SIRIUSLY_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromSiriusEcoSystemConfig(config: SiriusEcoSystemConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromSiriusEcoSystemConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.siriusEcoSystem${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".siriusEcoSystem", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".siriusEcoSystem", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.siriusEcoSystem"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: SiriusEcoSystemConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://siriusEcoSystem:siriusEcoSystem@127.0.0.1:${port}/siriusEcoSystem`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: SiriusEcoSystemConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.SIRIUSLY_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SIRIUSLY_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.SIRIUSLY_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.SIRIUSLY_SECRETS_MASTER_KEY_FILE) : null);
...
async function ensureEmbeddedPostgres(dataDir: string, preferredPort: number): Promise<EmbeddedPostgresHandle> {
  ...
  const instance = new EmbeddedPostgres({
    databaseDir: dataDir,
    user: "siriusEcoSystem",
    password: "siriusEcoSystem",
    port,
    persistent: true,
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: SiriusEcoSystemConfig;
  targetConfig: SiriusEcoSystemConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveSiriusEcoSystemEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readSiriusEcoSystemEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://siriusEcoSystem:siriusEcoSystem@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "siriusEcoSystem");
    const targetConnectionString = `postgres://siriusEcoSystem:siriusEcoSystem@127.0.0.1:${targetHandle.port}/siriusEcoSystem`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readSiriusEcoSystemEnvEntries(resolveSiriusEcoSystemEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.SIRIUSLY_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.SIRIUSLY_AGENT_JWT_SECRET);
  mergeSiriusEcoSystemEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { SIRIUSLY_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadSiriusEcoSystemEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run SiriusEcoSystem inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printSiriusEcoSystemCliBanner();
  p.intro(pc.bgCyan(pc.black(" sirius-eco-system worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printSiriusEcoSystemCliBanner();
  p.intro(pc.bgCyan(pc.black(" sirius-eco-system worktree:make ")));
...
type MergeSourceChoice = {
  ...
  branch: string | null;
  branchLabel: string;
  hasSiriusEcoSystemConfig: boolean;
  isCurrent: boolean;
};
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasSiriusEcoSystemConfig: existsSync(path.resolve(worktreePath, ".siriusEcoSystem", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printSiriusEcoSystemCliBanner();
  p.intro(pc.bgCyan(pc.black(" sirius-eco-system worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveSiriusEcoSystemEnvFile(configPath);
  const envEntries = readSiriusEcoSystemEnvEntries(envPath);
  const out = {
    SIRIUSLY_CONFIG: configPath,
    ...(envEntries.SIRIUSLY_HOME ? { SIRIUSLY_HOME: envEntries.SIRIUSLY_HOME } : {}),
    ...(envEntries.SIRIUSLY_INSTANCE_ID ? { SIRIUSLY_INSTANCE_ID: envEntries.SIRIUSLY_INSTANCE_ID } : {}),
    ...(envEntries.SIRIUSLY_CONTEXT ? { SIRIUSLY_CONTEXT: envEntries.SIRIUSLY_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasSiriusEcoSystemConfig)
      .map((choice) => path.resolve(choice.worktree, ".siriusEcoSystem", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readSiriusEcoSystemEnvEntries(resolveSiriusEcoSystemEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start SiriusEcoSystem once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasSiriusEcoSystemConfig ? "siriusEcoSystem" : "no-sirius-eco-system-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".siriusEcoSystem", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".siriusEcoSystem", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .siriusEcoSystem/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasSiriusEcoSystemConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a SiriusEcoSystem worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasSiriusEcoSystemConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No SiriusEcoSystem worktrees were found. Run `sirius-eco-system worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target SiriusEcoSystem configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local SiriusEcoSystem instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated SiriusEcoSystem instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with sirius-eco-system- if needed (created at ~/sirius-eco-system-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: SIRIUSLY_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: SIRIUSLY_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source SIRIUSLY_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local SiriusEcoSystem instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like SiriusEcoSystem worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with sirius-eco-system- if needed")
