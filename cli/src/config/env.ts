...
const JWT_SECRET_ENV_KEY = "SIRIUSLY_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by SiriusEcoSystem CLI commands",
...
export function resolveSiriusEcoSystemEnvFile(configPath?: string): string {
...
export function loadSiriusEcoSystemEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeSiriusEcoSystemEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readSiriusEcoSystemEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeSiriusEcoSystemEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeSiriusEcoSystemEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readSiriusEcoSystemEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeSiriusEcoSystemEnvEntries(next, filePath);
  return next;
}
