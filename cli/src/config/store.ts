...
import { siriusEcoSystemConfigSchema, type SiriusEcoSystemConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".siriusEcoSystem", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveSiriusEcoSystemInstanceId());
}
...
export function readConfig(configPath?: string): SiriusEcoSystemConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = siriusEcoSystemConfigSchema.safeParse(migrated);
  if (!parsed.success) {
