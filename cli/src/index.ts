...
const DATA_DIR_OPTION_HELP =
  "SiriusEcoSystem data directory root (isolates state from ~/.siriusEcoSystem)";

program
  .name("siriusly")
  .description("SiriusEcoSystem CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadSiriusEcoSystemEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start SiriusEcoSystem immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your SiriusEcoSystem setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "siriusEcoSystem")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run SiriusEcoSystem")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the SiriusEcoSystem server API")
