...
ClipHub is the public registry where people share, discover, and download SiriusEcoSystem company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to SiriusEcoSystem what a package registry is to a programming language. SiriusEcoSystem already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in SiriusEcoSystem — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own SiriusEcoSystem instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the SiriusEcoSystem spec. It contains:
...
**Install (fresh start):**
```
sirius-eco-system install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local SiriusEcoSystem instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within SiriusEcoSystem, export your company as a template, then publish:

```
sirius-eco-system export --template my-company
sirius-eco-system publish cliphub my-company
```
...
```
sirius-eco-system cliphub sync
```
...
ClipHub is a **separate service** from SiriusEcoSystem itself. SiriusEcoSystem is self-hosted; ClipHub is a hosted registry that SiriusEcoSystem instances talk to.
...
| **SiriusEcoSystem CLI** | `sirius-eco-system install`, `sirius-eco-system publish`, `sirius-eco-system cliphub sync` — built into SiriusEcoSystem |
| **SiriusEcoSystem UI** | "Browse ClipHub" panel in the SiriusEcoSystem web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with SiriusEcoSystem) |
| Backend | TypeScript + Hono (consistent with SiriusEcoSystem) |
| Database | PostgreSQL |
...
4. Run `sirius-eco-system install cliphub:acme/lean-saas-shop`
5. SiriusEcoSystem creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in SiriusEcoSystem until it works well
2. Export: `sirius-eco-system export --template my-agency`
3. Publish: `sirius-eco-system publish cliphub my-agency`
...
3. Install just that agent: `sirius-eco-system install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to SiriusEcoSystem

ClipHub is **not required** to use SiriusEcoSystem. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to SiriusEcoSystem what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `sirius-eco-system install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in SiriusEcoSystem web UI
- [ ] `sirius-eco-system cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with SiriusEcoSystem's own philosophy)
