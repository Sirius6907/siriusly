...
SiriusEcoSystem's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends SiriusEcoSystem's API, UI, and data model | Yes |
...
modules/
  observability/
    sirius-eco-system.module.json     # manifest (required)
    src/
...
### Manifest (`sirius-eco-system.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@sirius-eco-system/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@sirius-eco-system/core";
...
```
1. Scan modules/ directory for sirius-eco-system.module.json manifests
...
```jsonc
// sirius-eco-system.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@sirius-eco-system/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@sirius-eco-system/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:sirius-eco-system/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.sirius-eco-system.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm sirius-eco-system store list                    # browse available modules and templates
pnpm sirius-eco-system store install <module-id>     # install a module
pnpm sirius-eco-system store import <template-id>    # import a company template
pnpm sirius-eco-system store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one SiriusEcoSystem instance | `server:started` |
...
Add to `@sirius-eco-system/server`:
...
4. **Module config** — `sirius-eco-system.config.json` with per-module config, env var interpolation
...
10. **`@sirius-eco-system/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
