...
- New config/env settings:
  - `SIRIUSLY_AGENT_JWT_SECRET`
  - `SIRIUSLY_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `SIRIUSLY_AGENT_JWT_ISSUER` (default: `sirius-eco-system`)
  - `SIRIUSLY_AGENT_JWT_AUDIENCE` (default: `sirius-eco-system-api`)
...
1. In:
   ...
   inject `SIRIUSLY_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `SIRIUSLY_API_KEY`, do not overwrite it.
- Continue injecting:
  - `SIRIUSLY_AGENT_ID`
  - `SIRIUSLY_COMPANY_ID`
  - `SIRIUSLY_API_URL`
...
- Local adapters authenticate without manual `SIRIUSLY_API_KEY` config.
