...
Agents need API keys to authenticate with SiriusEcoSystem. The current approach
...
**Trust model:** The adapter process runs on the same machine as the SiriusEcoSystem
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** SiriusEcoSystem generates a token and passes it directly to the agent
...
`SIRIUSLY_API_URL`, `SIRIUSLY_AGENT_ID`, `SIRIUSLY_COMPANY_ID`. We need to
add a `SIRIUSLY_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a SiriusEcoSystem CLI
...
```
sirius-eco-system auth login
# Opens browser -> user confirms -> token stored at ~/.sirius-eco-system/credentials
```
...
managed by the SiriusEcoSystem server itself.
...
1. A company admin (user or agent) generates an **invite URL** from SiriusEcoSystem.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The SiriusEcoSystem SKILL.md (or a link to it)
   - What information SiriusEcoSystem needs from the agent (e.g. webhook URL, adapter
...
5. SiriusEcoSystem stores the pending registration.
...
7. On approval, SiriusEcoSystem provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** SiriusEcoSystem issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.sirius-eco-system.ing/skills/sirius-eco-system/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How SiriusEcoSystem should send you heartbeats",
...
1. Generate a SiriusEcoSystem invite link for the company.
...
4. A SiriusEcoSystem company member approves the new agent.
5. SiriusEcoSystem begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `SIRIUSLY_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `sirius-eco-system auth login` for developer-managed remote agents.                                      |
