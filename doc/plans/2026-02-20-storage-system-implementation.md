...
Add a single storage subsystem for SiriusEcoSystem that supports:
...
- API serves attachment bytes through authenticated SiriusEcoSystem endpoints.
...
- Default local path is under instance root: `~/.sirius-eco-system/instances/<instanceId>/data/storage`.
...
- `sirius-eco-system onboard` writes a valid `storage` config block by default.
- `sirius-eco-system configure --section storage` can switch between local and s3 modes.
...
- `sirius-eco-system doctor` reports actionable storage status.
