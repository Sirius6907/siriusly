...
- local mode that still feels instant (`npx siriuslyai run` and go)
...
2. CLI command `pnpm sirius-eco-system auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm sirius-eco-system onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm sirius-eco-system onboard`).
...
- expose key instance settings in API + CLI (`sirius-eco-system configure` / `sirius-eco-system onboard`)
...
7. `pnpm sirius-eco-system onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
