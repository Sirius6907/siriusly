...
This document defines the V1 design for a SiriusEcoSystem adapter that integrates with
...
4. synthesized stdout events for SiriusEcoSystem UI/CLI
...
3. **Agent callback auth to SiriusEcoSystem** uses a bootstrap exchange flow (no long-lived SiriusEcoSystem key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from SiriusEcoSystem endpoints, not full SKILL.md prompt injection.
...
- `siriusEcoSystemPublicUrl` (optional override; else `SIRIUSLY_PUBLIC_URL` env)
...
## SiriusEcoSystem Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SIRIUSLY_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.siriusEcoSystemPublicUrl`
2. `process.env.SIRIUSLY_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived SiriusEcoSystem credentials in prompt text.

Flow:

1. Before launch/follow-up, SiriusEcoSystem mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `siriusEcoSystemPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. SiriusEcoSystem validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all SiriusEcoSystem API calls.
...
1. Prompt includes a compact instruction to fetch skills from SiriusEcoSystem.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/sirius-eco-system`
   - `GET /api/skills/sirius-eco-system-create-agent` when needed
...
- resolve `siriusEcoSystemPublicUrl`
...
- include webhook object when enabled:
  - `url: <siriusEcoSystemPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active SiriusEcoSystem run context.
...
5. `siriusEcoSystemPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- sirius-eco-system public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from SiriusEcoSystem skill endpoints |
| SiriusEcoSystem auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from SiriusEcoSystem context.
3. Add richer PR metadata surfacing in SiriusEcoSystem UI.
