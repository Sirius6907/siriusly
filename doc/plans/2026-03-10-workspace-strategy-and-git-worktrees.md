...
`PAP-447` asks how SiriusEcoSystem should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same SiriusEcoSystem instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into SiriusEcoSystem:

- some operators use SiriusEcoSystem to manage SiriusEcoSystem and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as SiriusEcoSystem
- Claude and Codex expose different built-in affordances, so SiriusEcoSystem should not overfit to one tool

## Core Product Decision

SiriusEcoSystem should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by SiriusEcoSystem
...
- SiriusEcoSystem should not assume that every adapter can see or use a host filesystem path directly
...
If SiriusEcoSystem makes worktrees agent-first, it will blur:
...
- SiriusEcoSystem core resolves and records execution workspace state
...
That gives SiriusEcoSystem room to support:
...
- SiriusEcoSystem handles workspace runtime services under the hood using project/workspace policy
...
Once SiriusEcoSystem is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So SiriusEcoSystem should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that SiriusEcoSystem cannot infer
...
- when issue moves to done, should SiriusEcoSystem auto-commit?
- should it auto-push?
...
SiriusEcoSystem already has the right foundation for a project-first model.
...
For server-spawned local adapters, SiriusEcoSystem already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that SiriusEcoSystem should mirror directly

Implication:

- for `codex_local`, SiriusEcoSystem should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but SiriusEcoSystem should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, SiriusEcoSystem should separate:
...
SiriusEcoSystem should be able to express intentions such as:
...
These are not specific to SiriusEcoSystem. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".sirius-eco-system/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a SiriusEcoSystem-specific `devServer` flag.
...
SiriusEcoSystem should distinguish between:
...
SiriusEcoSystem should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### SiriusEcoSystem Core

SiriusEcoSystem core should:
...
SiriusEcoSystem core should not:
...
In addition to the local git helper, SiriusEcoSystem should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to SiriusEcoSystem
...
For runtime services specifically, SiriusEcoSystem should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as SiriusEcoSystem-only dev-server flags
...
3. Inject resolved execution workspace details into `context.siriusEcoSystemWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / SiriusEcoSystem instance can be reused through env/config injection when appropriate
...
- SiriusEcoSystem can decide whether to reuse or start a fresh service deterministically
...
3. Test against SiriusEcoSystem-on-SiriusEcoSystem workflows first.
...
7. Workspace runtime services are modeled generically, not as SiriusEcoSystem-specific dev-server toggles.
