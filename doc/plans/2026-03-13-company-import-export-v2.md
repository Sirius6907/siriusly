...
This document defines the next-stage plan for SiriusEcoSystem company import/export.

The core shift is:

- move from a SiriusEcoSystem-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside SiriusEcoSystem.
...
SiriusEcoSystem already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just SiriusEcoSystem
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `sirius-eco-system.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.sirius-eco-system.yaml` sidecar for high-fidelity SiriusEcoSystem-specific details
...
11. adapter-aware skill sync surfaces so SiriusEcoSystem can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point SiriusEcoSystem at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a SiriusEcoSystem sidecar.
...
SiriusEcoSystem must not redefine `SKILL.md`.
...
- SiriusEcoSystem-specific fidelity lives in `.sirius-eco-system.yaml`
- SiriusEcoSystem may resolve and install `SKILL.md` packages, but it must not require a SiriusEcoSystem-only skill format
...
### 5.4 Base Package Vs SiriusEcoSystem Extension
...
- base package:
  ...
  - no SiriusEcoSystem-only runtime fields by default
- SiriusEcoSystem extension:
  - `.sirius-eco-system.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by SiriusEcoSystem tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`sirius-eco-system.manifest.json` is not part of the future package direction.
...
SiriusEcoSystem import/export should support these entity kinds:
...
In SiriusEcoSystem V2 portability:
...
- SiriusEcoSystem can treat “this set of agents came from team package X” as the imported-team model
...
SiriusEcoSystem should support source references in package metadata with:
...
SiriusEcoSystem should define a new adapter surface area around skills:
...
Planned SiriusEcoSystem behavior:

- if an adapter supports read, SiriusEcoSystem should show current skills in the UI
- if an adapter supports write, SiriusEcoSystem should let the user enable/disable imported skills
- if an adapter supports sync, SiriusEcoSystem should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.sirius-eco-system.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.sirius-eco-system.yaml` alongside the base package
...
## 10. Storage Model Inside SiriusEcoSystem
...
SiriusEcoSystem should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `sirius-eco-system company export <company-id> --out <path>`
- `sirius-eco-system company import <path-or-url> --dry-run`
- `sirius-eco-system company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in SiriusEcoSystem storage, or only referenced at import time?
...
3. Should SiriusEcoSystem support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps SiriusEcoSystem aligned with:
