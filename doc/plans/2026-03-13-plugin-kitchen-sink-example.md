...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented SiriusEcoSystem plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside SiriusEcoSystem.
...
- package: `@sirius-eco-system/plugin-kitchen-sink-example`
- plugin id: `sirius-eco-system.kitchen-sink-example` or `sirius-eco-system-kitchen-sink-example`

Recommendation: use `sirius-eco-system-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core SiriusEcoSystem event such as `issue.created`
...
Recommendation: default to a SiriusEcoSystem-local endpoint or a stable public echo endpoint to avoid flaky docs.
