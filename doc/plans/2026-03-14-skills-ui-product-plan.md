...
This document defines the product and UI plan for skill management in SiriusEcoSystem.
...
- SiriusEcoSystem company import/export can include skills as package content
...
3. The UI must always tell the truth about what SiriusEcoSystem knows:
   - desired state in SiriusEcoSystem
...
SiriusEcoSystem should treat skills at two scopes:
...
- created locally inside SiriusEcoSystem later
...
7. “Understand what SiriusEcoSystem knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a SiriusEcoSystem-specific package layout
...
This matters because Codex and similar adapters may already have local skills that SiriusEcoSystem did not install.

These should be clearly marked:

- external
- not managed by SiriusEcoSystem
...
- `desired_only`: SiriusEcoSystem wants it, adapter does not show it yet
- `external`: adapter has it but SiriusEcoSystem does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, SiriusEcoSystem only tracks desired state
...
- SiriusEcoSystem can still track desired skills
...
- SiriusEcoSystem can see adapter skills
...
- `.sirius-eco-system.yaml` may add SiriusEcoSystem-specific fidelity, but should not replace the base shortname association model
...
- `.sirius-eco-system.yaml` may add SiriusEcoSystem-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
SiriusEcoSystem should have a company-scoped skill model or managed package model representing:
...
SiriusEcoSystem should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too SiriusEcoSystem-specific.
...
That gives SiriusEcoSystem one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
