...
Today release smoke testing for published SiriusEcoSystem packages is manual and shell-driven:
...
- Docker
- `npx siriuslyai@canary`
- `npx siriuslyai@latest`
...
- runs `sirius-eco-system@${SIRIUSLYAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@sirius-eco-system.local`
- password: `sirius-eco-system-smoke-password`
...
- `sirius-eco-system_version=canary`
...
- `sirius-eco-system_version=latest`
...
- the suite passes locally against both:
  - `SIRIUSLYAI_VERSION=canary`
  - `SIRIUSLYAI_VERSION=latest`
...
1. A published `sirius-eco-system@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `sirius-eco-system@latest` Docker install can be smoke-tested by Playwright in CI.
