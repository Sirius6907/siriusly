...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/sirius-eco-system-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `SIRIUSLY_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `SIRIUSLY_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - SiriusEcoSystem may create host-local worktrees, processes, and paths
- `cloud`
  - SiriusEcoSystem should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `sirius-eco-system | github | vercel | s3 | custom`
...
- use `SIRIUSLY_EXECUTION_TOPOLOGY` as a defaulting guardrail
