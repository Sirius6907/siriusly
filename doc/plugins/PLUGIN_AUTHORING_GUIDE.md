...
This guide describes the current, implemented way to create a SiriusEcoSystem plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main SiriusEcoSystem app.
...
node packages/plugins/create-sirius-eco-system-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the SiriusEcoSystem repo:

```bash
pnpm --filter @sirius-eco-system/create-sirius-eco-system-plugin build
node packages/plugins/create-sirius-eco-system-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@sirius-eco-system/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@sirius-eco-system/plugin-sdk` from the local SiriusEcoSystem checkout into a `.sirius-eco-system-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into SiriusEcoSystem from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@sirius-eco-system/plugin-sdk/ui`
