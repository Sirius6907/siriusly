# SiriusEcoSystem Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for SiriusEcoSystem's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main SiriusEcoSystem app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary SiriusEcoSystem HTTP APIs directly.
...
SiriusEcoSystem plugin design is based on the following assumptions:

1. SiriusEcoSystem is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core SiriusEcoSystem business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by SiriusEcoSystem core.
...
2. Let plugins add major capabilities without editing SiriusEcoSystem core.
...
4. Depend on project-local plugin folders such as `.sirius-eco-system/plugins`.
...
The single SiriusEcoSystem deployment an operator installs and controls.

### 5.2 Company

A first-class SiriusEcoSystem business object inside the instance.
...
A trusted in-process extension loaded directly by SiriusEcoSystem core.
...
An installable instance-wide extension package loaded through the SiriusEcoSystem plugin runtime.
...
SiriusEcoSystem has two extension classes.
...
SiriusEcoSystem already has a concrete workspace model:
...
Plugins live under the SiriusEcoSystem instance directory.

Suggested layout:

- `~/.sirius-eco-system/instances/default/plugins/package.json`
- `~/.sirius-eco-system/instances/default/plugins/node_modules/`
- `~/.sirius-eco-system/instances/default/plugins/.cache/`
- `~/.sirius-eco-system/instances/default/data/plugins/<plugin-id>/`
...
SiriusEcoSystem should add CLI commands:

- `pnpm sirius-eco-system plugin list`
- `pnpm sirius-eco-system plugin install <package[@version]>`
- `pnpm sirius-eco-system plugin uninstall <plugin-id>`
- `pnpm sirius-eco-system plugin upgrade <plugin-id> [version]`
- `pnpm sirius-eco-system plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@sirius-eco-system/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@sirius-eco-system/plugin-linear",
  "version": "0.1.0",
  "siriusEcoSystemPlugin": {
...
```ts
export interface SiriusEcoSystemPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumSiriusEcoSystemVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that SiriusEcoSystem agents can use during runs.
...
- SiriusEcoSystem server starts one worker process per installed plugin
...
Receives one typed SiriusEcoSystem domain event.
...
export function definePlugin(definition: PluginDefinition): SiriusEcoSystemPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: SiriusEcoSystemPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@sirius-eco-system/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@sirius-eco-system/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@sirius-eco-system/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@sirius-eco-system/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@sirius-eco-system/plugin-sdk/ui`
...
The `@sirius-eco-system/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the SiriusEcoSystem secret provider system rather than a plain text input
...
1. Core SiriusEcoSystem data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing SiriusEcoSystem objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual SiriusEcoSystem product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if SiriusEcoSystem later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing SiriusEcoSystem secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm sirius-eco-system plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the SiriusEcoSystem server. This is a normative requirement, not optional.
...
### 27.1 `@sirius-eco-system/plugin-test-harness`
...
```ts
import { createTestHarness } from "@sirius-eco-system/plugin-test-harness";
...
For developing a plugin against a running SiriusEcoSystem instance:

- The operator installs the plugin from a local path: `pnpm sirius-eco-system plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-sirius-eco-system-plugin`) that scaffolds:

- `package.json` with correct `siriusEcoSystemPlugin` keys
...
- `@sirius-eco-system/plugin-workspace-files`
- `@sirius-eco-system/plugin-terminal`
- `@sirius-eco-system/plugin-git`
- `@sirius-eco-system/plugin-linear`
- `@sirius-eco-system/plugin-github-issues`
- `@sirius-eco-system/plugin-grafana`
- `@sirius-eco-system/plugin-runtime-processes`
- `@sirius-eco-system/plugin-stripe`
...
- `@sirius-eco-system/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@sirius-eco-system/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@sirius-eco-system/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@sirius-eco-system/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@sirius-eco-system/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@sirius-eco-system/plugin-sdk/ui`
...
- `@sirius-eco-system/plugin-test-harness`
- `create-sirius-eco-system-plugin` starter template
...
SiriusEcoSystem should not implement a generic in-process hook bag modeled directly after local coding tools.

SiriusEcoSystem should implement:
...
That is the complete target design for the SiriusEcoSystem plugin system.
