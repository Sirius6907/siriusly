...
SiriusEcoSystem V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a SiriusEcoSystem-shaped design.

Assumption for this document: SiriusEcoSystem is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class SiriusEcoSystem objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into SiriusEcoSystem.

The main conclusion is:

- SiriusEcoSystem should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- SiriusEcoSystem should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- SiriusEcoSystem should use multiple extension classes instead of one generic plugin bag:
...
If SiriusEcoSystem does this well, the examples you listed become straightforward:
...
Relevant SiriusEcoSystem files reviewed for current extension seams:
...
It is too dangerous for SiriusEcoSystem core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for SiriusEcoSystem — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
SiriusEcoSystem should absolutely do this.
...
SiriusEcoSystem should copy this discipline.
...
SiriusEcoSystem should adopt this style for plugin actions, automations, and UI schemas.
...
SiriusEcoSystem can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for SiriusEcoSystem too.

## What SiriusEcoSystem Should Not Copy Directly
...
SiriusEcoSystem is a control plane for an operator-managed instance with company objects.
...
SiriusEcoSystem is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

SiriusEcoSystem should not auto-load arbitrary code from a workspace repo like `.sirius-eco-system/plugins` or project directories.
...
For SiriusEcoSystem, equivalent hooks into:
...
SiriusEcoSystem should not allow plugins to silently replace:
...
For SiriusEcoSystem it would be risky because it combines:
...
SiriusEcoSystem should require an explicit operator install step.

## Why SiriusEcoSystem Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | SiriusEcoSystem |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted SiriusEcoSystem instance |
...
That means SiriusEcoSystem should borrow the good ideas from `opencode` but use a stricter architecture.

## SiriusEcoSystem Already Has Useful Pre-Plugin Seams

SiriusEcoSystem has several extension-like seams already:
...
This is good news.
SiriusEcoSystem does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended SiriusEcoSystem Plugin Model
...
SiriusEcoSystem already has this pattern implicitly:
...
- `plugin` means integration code that talks to SiriusEcoSystem through a typed plugin protocol and capability model
...
- invoke explicit SiriusEcoSystem actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@sirius-eco-system/plugin-sdk/ui`) with design tokens and common components.
...
SiriusEcoSystem should treat plugin installation as a global instance-level action.

Examples:

- install `@sirius-eco-system/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over SiriusEcoSystem objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

SiriusEcoSystem already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for SiriusEcoSystem too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@sirius-eco-system/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@sirius-eco-system/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running SiriusEcoSystem instance.

A `create-sirius-eco-system-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how SiriusEcoSystem achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the SiriusEcoSystem server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. SiriusEcoSystem's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@sirius-eco-system/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@sirius-eco-system/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For SiriusEcoSystem

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@sirius-eco-system/plugin-sdk";

export default definePlugin({
  id: "@sirius-eco-system/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit SiriusEcoSystem entities
    });
    ...
    ctx.events.on("plugin.@sirius-eco-system/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@sirius-eco-system/plugin-sdk/ui";
...
Any SiriusEcoSystem plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific SiriusEcoSystem objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@sirius-eco-system/plugin-linear`)
...
- reuse core tables when the data is clearly part of SiriusEcoSystem itself
...
If a concept is becoming part of SiriusEcoSystem's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of SiriusEcoSystem itself
...
### 3. Scope plugin data to SiriusEcoSystem objects before adding custom schemas

A lot of plugin data naturally hangs off existing SiriusEcoSystem objects:
...
If SiriusEcoSystem eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@sirius-eco-system/plugin-workspace-files`
...
Package idea: `@sirius-eco-system/plugin-terminal`
...
Package idea: `@sirius-eco-system/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving SiriusEcoSystem.
- Board reviews the diff after a run without leaving SiriusEcoSystem.
...
- optional `events.emit` (e.g. `plugin.@sirius-eco-system/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@sirius-eco-system/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@sirius-eco-system/plugin-linear`

This plugin syncs SiriusEcoSystem work with Linear. It is useful for:

- importing backlog from Linear
- linking SiriusEcoSystem issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between SiriusEcoSystem and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into SiriusEcoSystem.
- SiriusEcoSystem issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@sirius-eco-system/plugin-github-issues`

This plugin syncs SiriusEcoSystem issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to SiriusEcoSystem issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with SiriusEcoSystem task governance
...
- Plugin settings:
  ...
  - whether new SiriusEcoSystem issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active SiriusEcoSystem issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into SiriusEcoSystem.
- GitHub webhooks update status/comment state in SiriusEcoSystem.
- A PR is linked back to the SiriusEcoSystem issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@sirius-eco-system/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@sirius-eco-system/plugin-grafana`

This plugin surfaces external metrics and dashboards inside SiriusEcoSystem. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating SiriusEcoSystem issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full SiriusEcoSystem plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the SiriusEcoSystem dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a SiriusEcoSystem issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of SiriusEcoSystem core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@sirius-eco-system/plugin-runtime-processes`
...
Package idea: `@sirius-eco-system/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into SiriusEcoSystem. It is useful for:
...
- Failed payment spikes or churn events can generate SiriusEcoSystem issues for follow-up.
...
- Stripe data should stay additive to SiriusEcoSystem core
...
- plugin UI bundle loading, host bridge, `@sirius-eco-system/plugin-sdk/ui`
...
- `@sirius-eco-system/plugin-test-harness` and `create-sirius-eco-system-plugin` starter template
...
SiriusEcoSystem should not implement "an OpenCode-style generic in-process hook system."
SiriusEcoSystem should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In SiriusEcoSystem
...
8. Ship `@sirius-eco-system/plugin-test-harness` and `create-sirius-eco-system-plugin` starter template.
