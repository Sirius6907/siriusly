...
This spec defines how SiriusEcoSystem actually runs agents while staying runtime-agnostic.
...
1. SiriusEcoSystem is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, SiriusEcoSystem manages lifecycle, and on exit SiriusEcoSystem parses JSON output and updates state.
...
```json
{
  "runLogStore": {
    ...
    "bucket": "sirius-eco-system-run-logs",
    "prefix": "runs/",
    "compress": true,
...
- `run.startedAt`
- `heartbeat.reason`
- `sirius-eco-system.skill` (shared SiriusEcoSystem skill text block)
...
2. Preferred transport is env vars (`SIRIUSLY_*`) injected at runtime.
