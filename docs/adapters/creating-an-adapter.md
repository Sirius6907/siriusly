...
Build a custom adapter to connect SiriusEcoSystem to any agent runtime.
...
2. Build environment with `buildSiriusEcoSystemEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `sirius-eco-system run --watch` using `picocolors`.
...
Make SiriusEcoSystem skills discoverable to your agent runtime without writing to the agent's working directory:
