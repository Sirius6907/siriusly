...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only SiriusEcoSystem.
...
- be useful outside SiriusEcoSystem
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.sirius-eco-system.yaml`
...
Example SiriusEcoSystem extension:
...
- SiriusEcoSystem must not require extra top-level fields for skill validity
- SiriusEcoSystem-specific extensions must live under `metadata.sirius-eco-system` or `metadata.sources`
...
For SiriusEcoSystem, the preferred fidelity extension is:
...
- issue/task SiriusEcoSystem-only metadata
...
- SiriusEcoSystem tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested SiriusEcoSystem shape:

```yaml
schema: sirius-eco-system/v1
agents:
  claudecoder:
    adapter:
      type: claude_local
...
Additional rules for SiriusEcoSystem exporters:
...
- omit empty and default-valued SiriusEcoSystem fields when possible
...
- SiriusEcoSystem exporters should emit `.sirius-eco-system.yaml` as a sidecar by default
...
## 19. SiriusEcoSystem Mapping

SiriusEcoSystem can map this spec to its runtime model like this:
...
- SiriusEcoSystem extension:
  - `.sirius-eco-system.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other SiriusEcoSystem-specific fidelity

Inline SiriusEcoSystem-only metadata that must live inside a shared markdown file should use:

- `metadata.sirius-eco-system`

That keeps the base format broader than SiriusEcoSystem.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only SiriusEcoSystem.

## 20. Cutover

SiriusEcoSystem should cut over to this markdown-first package model as the primary portability format.

`sirius-eco-system.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For SiriusEcoSystem, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
