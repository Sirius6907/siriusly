...
SiriusEcoSystem supports two runtime modes with different security profiles.
...
```sh
pnpm sirius-eco-system allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, SiriusEcoSystem emits a one-time claim URL at startup:
...
```sh
SIRIUSLY_DEPLOYMENT_MODE=authenticated pnpm sirius-eco-system run
```
