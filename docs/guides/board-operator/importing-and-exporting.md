...
SiriusEcoSystem companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .sirius-eco-system.yaml     # Adapter config, env inputs, routines
```
...
- **.sirius-eco-system.yaml** holds SiriusEcoSystem-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
sirius-eco-system company export <company-id> --out ./my-export
```
...
sirius-eco-system company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
sirius-eco-system company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
sirius-eco-system company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.sirius-eco-system.yaml`
...
```sh
# From a local folder
sirius-eco-system company import ./my-export

# From a GitHub URL
sirius-eco-system company import https://github.com/org/repo

# From a GitHub subfolder
sirius-eco-system company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
sirius-eco-system company import org/repo
sirius-eco-system company import org/repo/companies/acme
```
...
If `--target` is not specified, SiriusEcoSystem infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
sirius-eco-system company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
sirius-eco-system company import org/company-templates/engineering-team \
  --target new \
...
```sh
sirius-eco-system company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
sirius-eco-system company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
sirius-eco-system company import ./package \
  --target new \
  --yes \
  --json
```
...
SiriusEcoSystem supports several GitHub URL formats:
