...
Add first-class **documents** to SiriusEcoSystem as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by SiriusEcoSystem workflow/docs.
...
Update the SiriusEcoSystem agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/sirius-eco-system/SKILL.md`
