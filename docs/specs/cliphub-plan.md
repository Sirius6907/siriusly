# ClipHub: Marketplace for SiriusEcoSystem Team Configurations
...
> The "app store" for whole-company AI teams — pre-built SiriusEcoSystem configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for SiriusEcoSystem-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | SiriusEcoSystem company (agents, projects, governance) |
...
A complete SiriusEcoSystem company configuration:
...
Single-agent configurations designed to plug into a SiriusEcoSystem org:
...
Portable skill files that any SiriusEcoSystem agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with SiriusEcoSystem's skill loading system
...
interface Listing {
  ...
  siriusEcoSystemVersionMin: string;     // Minimum SiriusEcoSystem version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target SiriusEcoSystem company
...
| `POST` | `/api/listings/:id/install` | Install to SiriusEcoSystem company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target SiriusEcoSystem company (or create new)
  → ClipHub API calls SiriusEcoSystem API to:
  ...
  → Redirect to SiriusEcoSystem dashboard with new team running
```
...
### 5.3 Creator: Export from SiriusEcoSystem → Publish

```
Running SiriusEcoSystem company → "Export as Blueprint" (CLI or UI)
  → SiriusEcoSystem exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (SiriusEcoSystem brand blue/purple)
...
| Install flow | One-click deploy to SiriusEcoSystem company |
...
{
  "targetCompanyId": "uuid",         // Existing SiriusEcoSystem company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `sirius-eco-system-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as SiriusEcoSystem
- **Backend**: Node.js API (or extend SiriusEcoSystem server)
- **Database**: Postgres (can share SiriusEcoSystem's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with SiriusEcoSystem auth (or OAuth2)

### 9.2 Integration with SiriusEcoSystem

ClipHub can be:
- **Option A**: A separate app that calls SiriusEcoSystem's API to install blueprints
- **Option B**: A built-in section of the SiriusEcoSystem UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing SiriusEcoSystem UI and API.
...
- [ ] Install flow: blueprint → SiriusEcoSystem company
...
- [ ] "Export from SiriusEcoSystem" CLI command
