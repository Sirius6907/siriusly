...
const SIRIUSLY_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface SiriusEcoSystemSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: SiriusEcoSystemSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "siriusEcoSystem_required",
      originLabel: "Required by SiriusEcoSystem",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by SiriusEcoSystem",
    readOnly: false,
  };
}
...
export function buildSiriusEcoSystemEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.SIRIUSLY_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.SIRIUSLY_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.SIRIUSLY_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.SIRIUSLY_API_URL = apiUrl;
  return vars;
}
...
export async function resolveSiriusEcoSystemSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...SIRIUSLY_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listSiriusEcoSystemSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SiriusEcoSystemSkillEntry[]> {
  const root = await resolveSiriusEcoSystemSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `sirius-eco-system/siriusEcoSystem/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the SiriusEcoSystem skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "SiriusEcoSystem cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredSiriusEcoSystemRuntimeSkills(value: unknown): SiriusEcoSystemSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: SiriusEcoSystemSkillEntry[] = [];
...
export async function readSiriusEcoSystemRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<SiriusEcoSystemSkillEntry[]> {
  const configuredEntries = normalizeConfiguredSiriusEcoSystemRuntimeSkills(config.siriusEcoSystemRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listSiriusEcoSystemSkillEntries(moduleDir, additionalCandidates);
}

export async function readSiriusEcoSystemSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listSiriusEcoSystemSkillEntries(moduleDir);
...
export function readSiriusEcoSystemSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.siriusEcoSystemSkillSync;
...
function canonicalizeDesiredSiriusEcoSystemSkillReference(
  reference: string,
...
export function resolveSiriusEcoSystemDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readSiriusEcoSystemSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredSiriusEcoSystemSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeSiriusEcoSystemSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.siriusEcoSystemSkillSync;
  const current =
  ...
  next.siriusEcoSystemSkillSync = current;
  return next;
}

export async function ensureSiriusEcoSystemSkillSymlink(
  source: string,
  target: string,
  linkSkill: (source: string, target: string) => Promise<void> = async (linkSource, linkTarget) => {
    try {
      ...
      // On Windows, symlinks require admin privileges (EPERM/EISDIR).
      // Fall back to a recursive directory copy so skills work without elevation.
      const code = (err as NodeJS.ErrnoException).code;
  ...
  // If the target is a regular directory (not a symlink), check if it was
  // previously copied (Windows fallback) by verifying a known file exists.
  // Treat it as equivalent to a valid symlink — skip silently.
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the SiriusEcoSystem server itself is started from
    // within a Claude Code session (e.g. `npx siriuslyai run` in a terminal
