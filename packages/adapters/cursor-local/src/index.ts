...
Use when:
- You want SiriusEcoSystem to run Cursor Agent CLI locally as the agent runtime
...
- SiriusEcoSystem auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$siriusEcoSystem" and related skills on local runs.
- SiriusEcoSystem auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
