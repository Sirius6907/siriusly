...
This document describes how `@sirius-eco-system/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into SiriusEcoSystem logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (SiriusEcoSystem `runId`)
