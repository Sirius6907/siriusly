...
## Scope
This plan is now **gateway-only**. SiriusEcoSystem supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses SiriusEcoSystem skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a SiriusEcoSystem task.
...
```bash
pnpm --filter @sirius-eco-system/server test:run -- openclaw-gateway
pnpm --filter @sirius-eco-system/server typecheck
pnpm --filter @sirius-eco-system/ui typecheck
pnpm --filter sirius-eco-system typecheck
```
