...
Use when:
- You want SiriusEcoSystem to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the SiriusEcoSystem server.
...
- workspaceRuntime (object, optional): desired runtime service intents; SiriusEcoSystem forwards these in a standardized siriusEcoSystem.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- siriusEcoSystemApiUrl (string, optional): absolute SiriusEcoSystem base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default siriusEcoSystem)

Standard outbound payload additions:
- siriusEcoSystem (object): standardized SiriusEcoSystem context added to every gateway agent request
- siriusEcoSystem.workspace (object, optional): resolved execution workspace for this run
- siriusEcoSystem.workspaces (array, optional): additional workspace hints SiriusEcoSystem exposed to the run
- siriusEcoSystem.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
