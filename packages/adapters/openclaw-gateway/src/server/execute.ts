...
import { asNumber, asString, buildSiriusEcoSystemEnv, parseObject } from "@sirius-eco-system/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "siriusEcoSystem";
...
function resolveSiriusEcoSystemApiUrlOverride(value: unknown): string | null {
...
function buildSiriusEcoSystemEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const siriusEcoSystemApiUrlOverride = resolveSiriusEcoSystemApiUrlOverride(ctx.config.siriusEcoSystemApiUrl);
  const siriusEcoSystemEnv: Record<string, string> = {
    ...buildSiriusEcoSystemEnv(ctx.agent),
    SIRIUSLY_RUN_ID: ctx.runId,
  };

  if (siriusEcoSystemApiUrlOverride) {
    siriusEcoSystemEnv.SIRIUSLY_API_URL = siriusEcoSystemApiUrlOverride;
  }
  if (wakePayload.taskId) siriusEcoSystemEnv.SIRIUSLY_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) siriusEcoSystemEnv.SIRIUSLY_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) siriusEcoSystemEnv.SIRIUSLY_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) siriusEcoSystemEnv.SIRIUSLY_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) siriusEcoSystemEnv.SIRIUSLY_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    siriusEcoSystemEnv.SIRIUSLY_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return siriusEcoSystemEnv;
}

function buildWakeText(payload: WakePayload, siriusEcoSystemEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/sirius-eco-system-claimed-api-key.json";
  const orderedKeys = [
    "SIRIUSLY_RUN_ID",
    "SIRIUSLY_AGENT_ID",
    "SIRIUSLY_COMPANY_ID",
    "SIRIUSLY_API_URL",
    "SIRIUSLY_TASK_ID",
    "SIRIUSLY_WAKE_REASON",
    "SIRIUSLY_WAKE_COMMENT_ID",
    "SIRIUSLY_APPROVAL_ID",
    "SIRIUSLY_APPROVAL_STATUS",
    "SIRIUSLY_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = siriusEcoSystemEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = siriusEcoSystemEnv.SIRIUSLY_API_URL ?? "<set SIRIUSLY_API_URL>";

  const lines = [
    "SiriusEcoSystem wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `SIRIUSLY_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load SIRIUSLY_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $SIRIUSLY_API_KEY on every API call.",
    "- Use X-SiriusEcoSystem-Run-Id: $SIRIUSLY_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: SIRIUSLY_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$SIRIUSLY_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$SIRIUSLY_COMPANY_ID/issues?assigneeAgentId=$SIRIUSLY_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardSiriusEcoSystemPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  siriusEcoSystemEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateSiriusEcoSystem = parseObject(payloadTemplate.siriusEcoSystem);
  const workspace = asRecord(ctx.context.siriusEcoSystemWorkspace);
  const workspaces = Array.isArray(ctx.context.siriusEcoSystemWorkspaces)
    ? ctx.context.siriusEcoSystemWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.siriusEcoSystemRuntimeServiceIntents)
    ? ctx.context.siriusEcoSystemRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardSiriusEcoSystem: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: siriusEcoSystemEnv.SIRIUSLY_API_URL ?? null,
  };

  if (workspace) {
    standardSiriusEcoSystem.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardSiriusEcoSystem.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardSiriusEcoSystem.workspaceRuntime = {
  ...
  return {
    ...templateSiriusEcoSystem,
    ...standardSiriusEcoSystem,
  };
}
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "sirius-eco-system-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const siriusEcoSystemEnv = buildSiriusEcoSystemEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, siriusEcoSystemEnv);
  ...
  const siriusEcoSystemPayload = buildStandardSiriusEcoSystemPayload(ctx, wakePayload, siriusEcoSystemEnv, payloadTemplate);
