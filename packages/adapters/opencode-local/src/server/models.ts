...
import type { AdapterModel } from "@sirius-eco-system/adapter-utils";
import {
  asString,
  ensurePathInEnv,
  runChildProcess,
...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.SIRIUSLY_OPENCODE_COMMAND === "string" &&
    process.env.SIRIUSLY_OPENCODE_COMMAND.trim().length > 0
      ? process.env.SIRIUSLY_OPENCODE_COMMAND.trim()
      : "opencode";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["SIRIUSLY_", "npm_", "NPM_"] as const;
...
export async function ensureOpenCodeModelConfiguredAndAvailable(input: {
  ...
  if (models.length === 0 || !models.some((entry) => entry.id === model)) {
    console.warn(`[siriusEcoSystem] OpenCode model ${model} might not be available yet. Continuing anyway.`);
    return [{ id: model, label: model }, ...models];
  }
