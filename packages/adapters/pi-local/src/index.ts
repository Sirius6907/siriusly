...
Use when:
- You want SiriusEcoSystem to run Pi (the AI coding agent) locally as the agent runtime
...
- SiriusEcoSystem requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/siriusEcoSystems/ and resumed with --session.
