...
import { asString, runChildProcess } from "@sirius-eco-system/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.SIRIUSLY_PI_COMMAND === "string" &&
    process.env.SIRIUSLY_PI_COMMAND.trim().length > 0
      ? process.env.SIRIUSLY_PI_COMMAND.trim()
      : "pi";
