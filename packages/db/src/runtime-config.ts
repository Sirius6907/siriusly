...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "sirius-eco-system-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveSiriusEcoSystemHomeDir(), "instances", resolveSiriusEcoSystemInstanceId(), "db");
}
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".siriusEcoSystem", CONFIG_BASENAME);
...
function resolveSiriusEcoSystemConfigPath(): string {
  if (process.env.SIRIUSLY_CONFIG?.trim()) {
    return path.resolve(process.env.SIRIUSLY_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveSiriusEcoSystemEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveSiriusEcoSystemEnvPath(configPath);
