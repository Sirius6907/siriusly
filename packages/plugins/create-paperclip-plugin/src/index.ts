...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".sirius-eco-system-sdk");
...
/**
 * Generate a complete SiriusEcoSystem plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A SiriusEcoSystem plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "sirius-eco-system-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["siriusEcoSystem", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@sirius-eco-system/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@sirius-eco-system/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { SiriusEcoSystemPluginManifestV1 } from "@sirius-eco-system/plugin-sdk";

const manifest: SiriusEcoSystemPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@sirius-eco-system/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@sirius-eco-system/plugin-sdk\` and \`@sirius-eco-system/shared\` from a local SiriusEcoSystem checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.sirius-eco-system-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-sirius-eco-system-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <sirius-eco-system-sdk-path>]");
    process.exit(1);
  }
