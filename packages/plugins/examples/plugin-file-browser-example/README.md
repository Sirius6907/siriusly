...
Example SiriusEcoSystem plugin that demonstrates:
...
```bash
pnpm --filter @sirius-eco-system/plugin-file-browser-example build
pnpm sirius-eco-system plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm sirius-eco-system plugin uninstall sirius-eco-system-file-browser-example --force
```
...
- Optional: use `sirius-eco-system-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
