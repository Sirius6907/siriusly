# @sirius-eco-system/plugin-hello-world-example
...
- a minimal React widget rendered in the SiriusEcoSystem dashboard
...
```bash
pnpm --filter @sirius-eco-system/plugin-hello-world-example build
pnpm sirius-eco-system plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm sirius-eco-system plugin uninstall sirius-eco-system.hello-world-example --force` then  
  `pnpm sirius-eco-system plugin install ./packages/plugins/examples/plugin-hello-world-example`.
