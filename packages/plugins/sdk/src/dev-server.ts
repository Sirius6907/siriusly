 ...
 *
 * Endpoint summary:
 * - `GET /__siriusEcoSystem__/health` for diagnostics
 * - `GET /__siriusEcoSystem__/events` for hot-reload stream
...
export async function startPluginDevServer(options: PluginDevServerOptions = {}): Promise<PluginDevServer> {
  ...
  const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url === "/__siriusEcoSystem__/health") {
      sendJson(res, { ok: true, rootDir, uiDir });
      return;
    }

    if (url === "/__siriusEcoSystem__/events") {
      res.writeHead(200, {
