/**
 * `@sirius-eco-system/plugin-sdk` — SiriusEcoSystem plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@sirius-eco-system/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@sirius-eco-system/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @sirius-eco-system/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@sirius-eco-system/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@sirius-eco-system/plugin-sdk";
 *
