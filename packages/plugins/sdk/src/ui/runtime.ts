...
type GlobalBridge = typeof globalThis & {
  __siriusEcoSystemPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__siriusEcoSystemPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `SiriusEcoSystem plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`SiriusEcoSystem plugin UI component "${name}" is not callable`);
}
