...
/**
 * Zod schema for {@link SiriusEcoSystemPluginManifestV1} — the complete runtime
 ...
 * | `minimumSiriusEcoSystemVersion`| string?    | legacy alias of `minimumHostVersion`         |
 ...
 *
 * @see PLUGIN_SPEC.md §10.1 — Manifest shape
 * @see {@link SiriusEcoSystemPluginManifestV1} — the inferred TypeScript type
 */
export const pluginManifestV1Schema = z.object({
  ...
  minimumSiriusEcoSystemVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumSiriusEcoSystemVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumSiriusEcoSystemVersion
    && manifest.minimumHostVersion !== manifest.minimumSiriusEcoSystemVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumSiriusEcoSystemVersion must match when both are declared",
