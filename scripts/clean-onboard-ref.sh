...
Environment overrides:
  ...
  PC_DATA=/tmp/data           SiriusEcoSystem data dir to use
  SIRIUSLY_HOST=127.0.0.1    Host passed to the onboarded server
  SIRIUSLY_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/sirius-eco-system-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/sirius-eco-system-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export SIRIUSLY_HOST="${SIRIUSLY_HOST:-127.0.0.1}"
export SIRIUSLY_PORT="${SIRIUSLY_PORT:-3100}"
export SIRIUSLY_OPEN_ON_LISTEN="${SIRIUSLY_OPEN_ON_LISTEN:-false}"
...
echo "SIRIUSLY_HOST: $SIRIUSLY_HOST"
echo "SIRIUSLY_PORT: $SIRIUSLY_PORT"
...
env \
  ...
  HOST="$SIRIUSLY_HOST" \
  PORT="$SIRIUSLY_PORT" \
  SIRIUSLY_OPEN_ON_LISTEN="$SIRIUSLY_OPEN_ON_LISTEN" \
