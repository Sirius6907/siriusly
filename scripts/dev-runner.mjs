...
const devServerStatusFilePath = path.join(repoRoot, ".sirius-eco-system", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".sirius-eco-system/dev-server-status.json",
]);
...
const env = {
  ...process.env,
  SIRIUSLY_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.SIRIUSLY_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.SIRIUSLY_MIGRATION_PROMPT ??= "never";
  env.SIRIUSLY_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.SIRIUSLY_DEPLOYMENT_MODE = "authenticated";
  env.SIRIUSLY_DEPLOYMENT_EXPOSURE = "private";
  env.SIRIUSLY_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[sirius-eco-system] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[sirius-eco-system] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@sirius-eco-system/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[sirius-eco-system] Command failed with code ${status.code}: pnpm --filter @sirius-eco-system/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[sirius-eco-system] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.SIRIUSLY_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[sirius-eco-system] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[sirius-eco-system] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@sirius-eco-system/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[sirius-eco-system] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@sirius-eco-system/server", serverScript, ...forwardedArgs],
