#!/usr/bin/env bash
#
# Kill all local SiriusEcoSystem dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all sirius-eco-system dev processes
...
# Collect PIDs of node processes running from any sirius-eco-system directory.
# Matches paths like /Users/*/sirius-eco-system/... or /Users/*/sirius-eco-system-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/sirius-eco-system(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No SiriusEcoSystem dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} SiriusEcoSystem dev process(es):"
echo ""
