...
SIRIUSLY_HOST_PORT="${SIRIUSLY_HOST_PORT:-3100}"
SIRIUSLY_HOST_FROM_CONTAINER="${SIRIUSLY_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.sirius-eco-system-openclaw.override.yml"
...
detect_sirius-eco-system_base_url() {
  ...
  for candidate in "$SIRIUSLY_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${SIRIUSLY_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${SIRIUSLY_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
sirius-eco-system_base_url="$(detect_sirius-eco-system_base_url || true)"
...
SiriusEcoSystem URL for OpenClaw container:
EOF
  if [[ -n "$sirius-eco-system_base_url" ]]; then
    cat <<EOF
  $sirius-eco-system_base_url
  (Use this base URL for invite/onboarding links from inside OpenClaw Docker.)
EOF
  else
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${SIRIUSLY_HOST_PORT}
  (Do not use http://127.0.0.1:${SIRIUSLY_HOST_PORT} inside the container.)
  If SiriusEcoSystem rejects the host, run on host machine:
    pnpm sirius-eco-system allowed-hostname host.docker.internal
  Then restart SiriusEcoSystem and re-run this script.
EOF
  fi
  cat <<EOF
