...
SIRIUSLY_API_URL="${SIRIUSLY_API_URL:-http://127.0.0.1:3100}"
API_BASE="${SIRIUSLY_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-sirius-eco-system-smoke}"
...
SIRIUSLY_API_URL_FOR_OPENCLAW="${SIRIUSLY_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${SIRIUSLY_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${SIRIUSLY_AUTH_HEADER}" )
fi
if [[ -n "${SIRIUSLY_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${SIRIUSLY_COOKIE}" )
  SIRIUSLY_BROWSER_ORIGIN="${SIRIUSLY_BROWSER_ORIGIN:-${SIRIUSLY_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${SIRIUSLY_BROWSER_ORIGIN}" -H "Referer: ${SIRIUSLY_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set SIRIUSLY_COOKIE or SIRIUSLY_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg siriusEcoSystemApiUrl "$SIRIUSLY_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "sirius-eco-system",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        siriusEcoSystemApiUrl: $siriusEcoSystemApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/sirius-eco-system"
  local claimed_file="${workspace_dir}/sirius-eco-system-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/sirius-eco-system-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/sirius-eco-system-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/sirius-eco-system-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/sirius-eco-system"
    assert_status "200"
    {
      echo "SIRIUSLY_API_URL=${SIRIUSLY_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set SIRIUSLY_API_KEY=${AGENT_API_KEY} in your run context before running SiriusEcoSystem heartbeat steps."
...
main() {
  ...
  wait_http_ready "${SIRIUSLY_API_URL%/}/api/health" 15 || fail "SiriusEcoSystem API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "sirius-eco-system health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
