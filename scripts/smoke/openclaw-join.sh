...
COMPANY_ID="${COMPANY_ID:-${SIRIUSLY_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-sirius-eco-system-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-sirius-eco-system-openclaw-smoke}"
...
Provide one of:
  SIRIUSLY_AUTH_HEADER=\"Bearer <board-token>\"
  SIRIUSLY_COOKIE=\"<board-session-cookie>\"
...
log "checking SiriusEcoSystem health"
...
assert_status "200"
if ! grep -q "SiriusEcoSystem OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.sirius-eco-system.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
