...
describe("agent local JWT", () => {
  const secretEnv = "SIRIUSLY_AGENT_JWT_SECRET";
  const ttlEnv = "SIRIUSLY_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "SIRIUSLY_AGENT_JWT_ISSUER";
  const audienceEnv = "SIRIUSLY_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "siriusEcoSystem";
    process.env[audienceEnv] = "sirius-eco-system-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
