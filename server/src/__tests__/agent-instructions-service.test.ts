...
describe("agent instructions service", () => {
  const originalSiriusEcoSystemHome = process.env.SIRIUSLY_HOME;
  const originalSiriusEcoSystemInstanceId = process.env.SIRIUSLY_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalSiriusEcoSystemHome === undefined) delete process.env.SIRIUSLY_HOME;
    else process.env.SIRIUSLY_HOME = originalSiriusEcoSystemHome;
    if (originalSiriusEcoSystemInstanceId === undefined) delete process.env.SIRIUSLY_INSTANCE_ID;
    else process.env.SIRIUSLY_INSTANCE_ID = originalSiriusEcoSystemInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-home-");
    const externalRoot = await makeTempDir("sirius-eco-system-agent-instructions-external-");
    cleanupDirs.add(siriusEcoSystemHome);
    cleanupDirs.add(externalRoot);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("sirius-eco-system-agent-instructions-new-external-");
    cleanupDirs.add(siriusEcoSystemHome);
    cleanupDirs.add(externalRoot);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("sirius-eco-system-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-recover-");
    cleanupDirs.add(siriusEcoSystemHome);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("sirius-eco-system-agent-instructions-stale-root-");
    cleanupDirs.add(siriusEcoSystemHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("sirius-eco-system-agent-instructions-heal-write-stale-");
    cleanupDirs.add(siriusEcoSystemHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("sirius-eco-system-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(siriusEcoSystemHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const siriusEcoSystemHome = await makeTempDir("sirius-eco-system-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("sirius-eco-system-agent-instructions-partial-root-");
    cleanupDirs.add(siriusEcoSystemHome);
    cleanupDirs.add(staleRoot);
    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = "test-instance";
