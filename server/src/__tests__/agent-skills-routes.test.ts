...
describe("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockCompanySkillService.listRuntimeSkillEntries.mockResolvedValue([
      {
        key: "sirius-eco-system/siriusEcoSystem/siriusEcoSystem",
        runtimeName: "siriusEcoSystem",
        source: "/tmp/siriusEcoSystem",
        required: true,
    ...
    mockCompanySkillService.resolveRequestedSkillKeys.mockImplementation(
      async (_companyId: string, requested: string[]) =>
        requested.map((value) =>
          value === "siriusEcoSystem"
            ? "sirius-eco-system/siriusEcoSystem/siriusEcoSystem"
            : value,
        ),
    );
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["sirius-eco-system/siriusEcoSystem/siriusEcoSystem"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          siriusEcoSystemRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  });
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await request(createApp())
      ...
      .send({ desiredSkills: ["sirius-eco-system/siriusEcoSystem/siriusEcoSystem"] });
  ...
  it("canonicalizes desired skill references before syncing", async () => {
    ...
    const res = await request(createApp())
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["siriusEcoSystem"] });

    expect(res.status, JSON.stringify(res.body)).toBe(200);
    expect(mockCompanySkillService.resolveRequestedSkillKeys).toHaveBeenCalledWith("company-1", ["siriusEcoSystem"]);
    expect(mockAgentService.update).toHaveBeenCalledWith(
      expect.any(String),
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          siriusEcoSystemSkillSync: expect.objectContaining({
            desiredSkills: ["sirius-eco-system/siriusEcoSystem/siriusEcoSystem"],
          }),
        }),
      }),
