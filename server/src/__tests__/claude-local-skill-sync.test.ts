...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "sirius-eco-system/siriusEcoSystem/sirius-eco-system-create-agent";
  ...
  it("defaults to mounting all built-in SiriusEcoSystem skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === siriusEcoSystemKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat SiriusEcoSystem skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("sirius-eco-system-claude-user-skills-");
