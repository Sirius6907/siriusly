...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "siriusEcoSystem", LOGNAME: "siriusEcoSystem", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "siriusEcoSystem" }),
      },
    );

    expect(tokens).toEqual(["siriusEcoSystem", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "siriusEcoSystem" }, {
        userInfo: () => ({ username: "siriusEcoSystem" }),
      });

      expect(tokens).toEqual(["siriusEcoSystem", "custom-token"]);
    } finally {
  ...
  it("reports matches without leaking which token was searched", () => {
    ...
    const exitCode = runForbiddenTokenCheck({
      repoRoot: "/repo",
      tokens: ["siriusEcoSystem", "custom-token"],
      exec,
      log,
      error,
    });
