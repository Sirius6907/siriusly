...
import type { agents } from "@sirius-eco-system/db";
import { sessionCodec as codexSessionCodec } from "@sirius-eco-system/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[siriusEcoSystem] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables SiriusEcoSystem-managed rotation by default for codex and claude local", () => {
