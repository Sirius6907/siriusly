...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("SiriusEcoSystem OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested SiriusEcoSystem base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("siriusEcoSystemApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.siriusEcoSystemApiUrl");
    expect(text).toContain("~/.openclaw/workspace/sirius-eco-system-claimed-api-key.json");
    expect(text).toContain("SIRIUSLY_API_KEY");
