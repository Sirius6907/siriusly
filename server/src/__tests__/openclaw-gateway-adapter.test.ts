...
import { execute, testEnvironment } from "@sirius-eco-system/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@sirius-eco-system/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              wakeReason: "issue_assigned",
              issueIds: ["issue-123"],
              siriusEcoSystemWorkspace: {
                cwd: "/tmp/worktrees/pap-123",
                strategy: "git_worktree",
                branchName: "pap-123-test",
              },
              siriusEcoSystemWorkspaces: [
                {
                  id: "workspace-1",
                  cwd: "/tmp/project",
                },
              ],
              siriusEcoSystemRuntimeServiceIntents: [
                {
                  name: "preview",
                  lifecycle: "ephemeral",
                },
      ...
      expect(payload?.sessionKey).toBe("siriusEcoSystem:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("SIRIUSLY_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("SIRIUSLY_TASK_ID=task-123");
