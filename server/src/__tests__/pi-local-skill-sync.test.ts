...
describe("pi local skill sync", () => {
  ...
  it("reports configured SiriusEcoSystem skills and installs them into the Pi skills home", async () => {
    const home = await makeTempDir("sirius-eco-system-pi-skill-sync-");
    ...
    const after = await syncPiSkills(ctx, [siriusEcoSystemKey]);
    expect(after.entries.find((entry) => entry.key === siriusEcoSystemKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".pi", "agent", "skills", "siriusEcoSystem"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled SiriusEcoSystem skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("sirius-eco-system-pi-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncPiSkills(configuredCtx, [siriusEcoSystemKey]);
