...
import { resolveSiriusEcoSystemConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@sirius-eco-system/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "siriusEcoSystem@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "SiriusEcoSystem Test"]);
...
afterEach(async () => {
  ...
  delete process.env.SIRIUSLY_WORKTREES_DIR;
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".siriusEcoSystem", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$SIRIUSLY_WORKSPACE_BRANCH\" > .sirius-eco-system-provision-branch",
        "printf '%s\\n' \"$SIRIUSLY_WORKSPACE_BASE_CWD\" > .sirius-eco-system-provision-base",
        "printf '%s\\n' \"$SIRIUSLY_WORKSPACE_CREATED\" > .sirius-eco-system-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-eco-system-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-eco-system-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".sirius-eco-system-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".sirius-eco-system-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local SiriusEcoSystem config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const siriusEcoSystemHome = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(siriusEcoSystemHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.SIRIUSLY_HOME = siriusEcoSystemHome;
    process.env.SIRIUSLY_INSTANCE_ID = instanceId;
    process.env.SIRIUSLY_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/siriusEcoSystem"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".siriusEcoSystem", "config.json");
      const envPath = path.join(workspace.cwd, ".siriusEcoSystem", ".env");
      ...
      expect(envContents).toContain(`SIRIUSLY_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`SIRIUSLY_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`SIRIUSLY_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("SIRIUSLY_IN_WORKTREE=true");
      expect(envContents).toContain(
        `SIRIUSLY_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveSiriusEcoSystemConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-runtime-workspace-"));
  ...
  it("does not leak parent SiriusEcoSystem instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "siriusEcoSystemConfig: process.env.SIRIUSLY_CONFIG ?? null,",
          "siriusEcoSystemHome: process.env.SIRIUSLY_HOME ?? null,",
          "siriusEcoSystemInstanceId: process.env.SIRIUSLY_INSTANCE_ID ?? null,",
    ...
    process.env.SIRIUSLY_CONFIG = "/tmp/base-sirius-eco-system-config.json";
    process.env.SIRIUSLY_HOME = "/tmp/base-sirius-eco-system-home";
    process.env.SIRIUSLY_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/siriusEcoSystem";
    ...
    expect(captured.siriusEcoSystemConfig).toBeNull();
    expect(captured.siriusEcoSystemHome).toBeNull();
    expect(captured.siriusEcoSystemInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-runtime-sibling-"));
