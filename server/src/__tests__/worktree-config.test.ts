...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const siriusEcoSystemDir = path.join(worktreeRoot, ".siriusEcoSystem");
    const configPath = path.join(siriusEcoSystemDir, "config.json");
    const envPath = path.join(siriusEcoSystemDir, ".env");
    const sharedRoot = path.join(tempRoot, ".siriusEcoSystem", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".sirius-eco-system-worktrees");

    await fs.mkdir(siriusEcoSystemDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# SiriusEcoSystem environment variables",
        "SIRIUSLY_IN_WORKTREE=true",
        "SIRIUSLY_WORKTREE_NAME=PAP-884-ai-commits-component",
        "SIRIUSLY_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.SIRIUSLY_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.SIRIUSLY_WORKTREES_DIR = isolatedHome;
    delete process.env.SIRIUSLY_HOME;
    delete process.env.SIRIUSLY_INSTANCE_ID;
    delete process.env.SIRIUSLY_CONFIG;
    delete process.env.SIRIUSLY_CONTEXT;
    ...
    expect(repairedEnv).toContain(`SIRIUSLY_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('SIRIUSLY_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`SIRIUSLY_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`SIRIUSLY_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('SIRIUSLY_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.SIRIUSLY_HOME).toBe(isolatedHome);
    expect(process.env.SIRIUSLY_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# SiriusEcoSystem environment variables",
        "SIRIUSLY_IN_WORKTREE=true",
        "SIRIUSLY_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.SIRIUSLY_IN_WORKTREE = "true";
    process.env.SIRIUSLY_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.SIRIUSLY_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".sirius-eco-system-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".siriusEcoSystem", "worktrees");
    ...
    const siriusEcoSystemDir = path.join(currentWorktreeRoot, ".siriusEcoSystem");
    const configPath = path.join(siriusEcoSystemDir, "config.json");
    const envPath = path.join(siriusEcoSystemDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".siriusEcoSystem", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sirius-eco-system-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.SIRIUSLY_IN_WORKTREE = "true";
    process.env.SIRIUSLY_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.SIRIUSLY_HOME = isolatedHome;
    process.env.SIRIUSLY_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.SIRIUSLY_CONFIG = configPath;
