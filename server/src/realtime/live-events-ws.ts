...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@sirius-eco-system/db";
import type { DeploymentMode } from "@sirius-eco-system/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  siriusEcoSystemUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).siriusEcoSystemUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.siriusEcoSystemUpgradeContext = context;
