...
import type { DeploymentExposure, DeploymentMode } from "@sirius-eco-system/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "siriusEcoSystem" &&
    normalized !== "sirius-eco-system-create-agent" &&
    normalized !== "sirius-eco-system-create-plugin" &&
...
/** Resolve the SiriusEcoSystem repo skills directory (built-in / managed skills). */
function resolveSiriusEcoSystemSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
interface AvailableSkill {
  name: string;
  description: string;
  isSiriusEcoSystemManaged: boolean;
}
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const siriusEcoSystemSkillsDir = resolveSiriusEcoSystemSkillsDir();

  // Build set of SiriusEcoSystem-managed skill names
  const siriusEcoSystemSkillNames = new Set<string>();
  if (siriusEcoSystemSkillsDir) {
    try {
      for (const entry of fs.readdirSync(siriusEcoSystemSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) siriusEcoSystemSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isSiriusEcoSystemManaged: siriusEcoSystemSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.siriusEcoSystemApiUrl)) {
    const legacySiriusEcoSystemApiUrl = nonEmptyTrimmedString(input.siriusEcoSystemApiUrl);
    if (legacySiriusEcoSystemApiUrl) merged.siriusEcoSystemApiUrl = legacySiriusEcoSystemApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    siriusEcoSystemApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.siriusEcoSystemApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawSiriusEcoSystemApiUrl =
    typeof defaults.siriusEcoSystemApiUrl === "string"
      ? defaults.siriusEcoSystemApiUrl.trim()
      : "";
  if (rawSiriusEcoSystemApiUrl) {
    try {
      const parsedSiriusEcoSystemApiUrl = new URL(rawSiriusEcoSystemApiUrl);
      if (
        parsedSiriusEcoSystemApiUrl.protocol !== "http:" &&
        parsedSiriusEcoSystemApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_siriusEcoSystem_api_url_protocol",
          level: "warn",
          message: `siriusEcoSystemApiUrl must use http:// or https:// (got ${parsedSiriusEcoSystemApiUrl.protocol}).`
        });
      } else {
        normalized.siriusEcoSystemApiUrl = parsedSiriusEcoSystemApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_siriusEcoSystem_api_url_configured",
          level: "info",
          message: `siriusEcoSystemApiUrl set to ${parsedSiriusEcoSystemApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_siriusEcoSystem_api_url_invalid",
        level: "warn",
        message: `Invalid siriusEcoSystemApiUrl: ${rawSiriusEcoSystemApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your SiriusEcoSystem host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "SiriusEcoSystem is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm sirius-eco-system allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/siriusEcoSystem";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/sirius-eco-system-claimed-api-key.json and load SIRIUSLY_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: siriusEcoSystemApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the SiriusEcoSystem hostname is reachable and allowed via `pnpm sirius-eco-system allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this SiriusEcoSystem API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        name: "siriusEcoSystem",
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/siriusEcoSystem/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # SiriusEcoSystem OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          siriusEcoSystemApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, SiriusEcoSystem generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "siriusEcoSystemApiUrl": "https://sirius-eco-system-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in SiriusEcoSystem before key claim is allowed.
    ...
    - ~/.openclaw/workspace/sirius-eco-system-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/sirius-eco-system-claimed-api-key.json

    And set the SIRIUSLY_API_KEY and SIRIUSLY_API_URL in your environment variables as specified here:
    ...
    Then set SIRIUSLY_API_KEY and SIRIUSLY_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install SiriusEcoSystem skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your SIRIUSLY_API_URL to the top of your skill and note the path to your SIRIUSLY_API_URL
    ...
    ${
      ...
      "Ensure SiriusEcoSystem is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested SiriusEcoSystem base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.siriusEcoSystemApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm sirius-eco-system allowed-hostname <host>
      - then restart SiriusEcoSystem and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@siriusEcoSystem.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "siriusEcoSystem", path: "/api/skills/siriusEcoSystem" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "sirius-eco-system-create-agent",
          path: "/api/skills/sirius-eco-system-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              siriusEcoSystemApiUrl: req.body.siriusEcoSystemApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.siriusEcoSystemApiUrl &&
          !persistedDefaults.siriusEcoSystemApiUrl
        ) {
          missingPersistedFields.push("siriusEcoSystemApiUrl");
        }
