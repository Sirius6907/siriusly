...
import { agents as agentsTable, companies, heartbeatRuns } from "@sirius-eco-system/db";
...
import { runClaudeLogin } from "@sirius-eco-system/adapter-claude-local/server";
import {
...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@sirius-eco-system/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.SIRIUSLY_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeSiriusEcoSystemSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readSiriusEcoSystemSkillSyncPreference(
