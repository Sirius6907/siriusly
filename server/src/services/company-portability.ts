...
import type { Db } from "@sirius-eco-system/db";
import type {
...
import {
  readSiriusEcoSystemSkillSyncPreference,
  writeSiriusEcoSystemSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".siriusEcoSystem.yaml" || normalized === ".siriusEcoSystem.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const siriusEcoSystem = isPlainRecord(metadata?.siriusEcoSystem) ? metadata?.siriusEcoSystem as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.siriusEcoSystemSkillKey)
    ?? asString(siriusEcoSystem?.skillKey)
    ?? asString(siriusEcoSystem?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "siriusEcoSystem_bundled") {
    return `sirius-eco-system/siriusEcoSystem/${slug}`;
  }
...
type SiriusEcoSystemExtensionDoc = {
  schema?: string;
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "siriusEcoSystem" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; SiriusEcoSystem will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .siriusEcoSystem.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  siriusEcoSystemExtensionPath: string,
) {
  ...
  const extensionEntry = filtered[siriusEcoSystemExtensionPath];
  if (selectedFiles.has(siriusEcoSystemExtensionPath) && typeof extensionEntry === "string") {
    filtered[siriusEcoSystemExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findSiriusEcoSystemExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".siriusEcoSystem.yaml"] === "string") return ".siriusEcoSystem.yaml";
  if (typeof files[".siriusEcoSystem.yml"] === "string") return ".siriusEcoSystem.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.siriusEcoSystem.yaml") || entry.endsWith("/.siriusEcoSystem.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "siriusEcoSystem_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "sirius-eco-system/siriusEcoSystem",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/sirius-eco-system/siriusEcoSystem/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "siriusEcoSystem_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.siriusEcoSystemSkillKey = skill.key;
  metadata.siriusEcoSystem = {
    ...(isPlainRecord(metadata.siriusEcoSystem) ? metadata.siriusEcoSystem : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const siriusEcoSystemExtensionPath = findSiriusEcoSystemExtensionPath(normalizedFiles);
  const siriusEcoSystemExtension = siriusEcoSystemExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, siriusEcoSystemExtensionPath) ?? "")
    : {};
  const siriusEcoSystemCompany = isPlainRecord(siriusEcoSystemExtension.company) ? siriusEcoSystemExtension.company : {};
  const siriusEcoSystemSidebar = normalizePortableSidebarOrder(siriusEcoSystemExtension.sidebar);
  const siriusEcoSystemAgents = isPlainRecord(siriusEcoSystemExtension.agents) ? siriusEcoSystemExtension.agents : {};
  const siriusEcoSystemProjects = isPlainRecord(siriusEcoSystemExtension.projects) ? siriusEcoSystemExtension.projects : {};
  const siriusEcoSystemTasks = isPlainRecord(siriusEcoSystemExtension.tasks) ? siriusEcoSystemExtension.tasks : {};
  const siriusEcoSystemRoutines = isPlainRecord(siriusEcoSystemExtension.routines) ? siriusEcoSystemExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(siriusEcoSystemCompany.brandColor),
      logoPath: asString(siriusEcoSystemCompany.logoPath) ?? asString(siriusEcoSystemCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof siriusEcoSystemCompany.requireBoardApprovalForNewAgents === "boolean"
          ? siriusEcoSystemCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: siriusEcoSystemSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(siriusEcoSystemAgents[slug]) ? siriusEcoSystemAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(siriusEcoSystemProjects[slug]) ? siriusEcoSystemProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(siriusEcoSystemTasks[slug]) ? siriusEcoSystemTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(siriusEcoSystemRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(siriusEcoSystemRoutines[slug]) ? siriusEcoSystemRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".siriusEcoSystem.yaml" ||
          relative === ".siriusEcoSystem.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const siriusEcoSystemAgentsOut: Record<string, Record<string, unknown>> = {};
    const siriusEcoSystemProjectsOut: Record<string, Record<string, unknown>> = {};
    const siriusEcoSystemTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const siriusEcoSystemRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readSiriusEcoSystemSkillSyncPreference(
        ...
        siriusEcoSystemAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      siriusEcoSystemProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      siriusEcoSystemTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      siriusEcoSystemRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const siriusEcoSystemExtensionPath = ".siriusEcoSystem.yaml";
    const siriusEcoSystemAgents = Object.fromEntries(
      Object.entries(siriusEcoSystemAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusEcoSystemProjects = Object.fromEntries(
      Object.entries(siriusEcoSystemProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusEcoSystemTasks = Object.fromEntries(
      Object.entries(siriusEcoSystemTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const siriusEcoSystemRoutines = Object.fromEntries(
      Object.entries(siriusEcoSystemRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[siriusEcoSystemExtensionPath] = buildYamlFile(
      {
        schema: "siriusEcoSystem/v1",
        ...
        agents: Object.keys(siriusEcoSystemAgents).length > 0 ? siriusEcoSystemAgents : undefined,
        projects: Object.keys(siriusEcoSystemProjects).length > 0 ? siriusEcoSystemProjects : undefined,
        tasks: Object.keys(siriusEcoSystemTasks).length > 0 ? siriusEcoSystemTasks : undefined,
        routines: Object.keys(siriusEcoSystemRoutines).length > 0 ? siriusEcoSystemRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, siriusEcoSystemExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeSiriusEcoSystemSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
