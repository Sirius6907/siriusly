...
import { companySkills } from "@sirius-eco-system/db";
import { readSiriusEcoSystemSkillSyncPreference, writeSiriusEcoSystemSkillSyncPreference } from "@sirius-eco-system/adapter-utils/server-utils";
import type { SiriusEcoSystemSkillEntry } from "@sirius-eco-system/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@sirius-eco-system/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveSiriusEcoSystemInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("sirius-eco-system/siriusEcoSystem/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveSiriusEcoSystemInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "siriusEcoSystem_bundled") {
    return {
      editable: false,
      editableReason: "Bundled SiriusEcoSystem skills are read-only.",
      sourceLabel: "SiriusEcoSystem bundled",
      sourceBadge: "siriusEcoSystem",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "SiriusEcoSystem workspace",
        sourceBadge: "siriusEcoSystem",
...
export function companySkillService(db: Db) {
        ...
        if (!adapter?.listSkills) {
          ...
          try {
            ...
            const snapshot = await adapter.listSkills({
              ...
              config: {
                ...runtimeConfig,
                siriusEcoSystemRuntimeSkills: runtimeSkillEntries,
              },
            });
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "siriusEcoSystem_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled SiriusEcoSystem skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "siriusEcoSystem_bundled"
        && incomingKind === "github"
        && incomingOwner === "sirius-eco-system"
        && incomingRepo === "siriusEcoSystem"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeSiriusEcoSystemSkillSyncPreference(config, filtered),
        });
      }
    }
