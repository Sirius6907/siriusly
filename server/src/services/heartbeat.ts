...
import type { BillingType } from "@sirius-eco-system/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_siriusEcoSystemWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__siriusEcoSystem_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function evaluateSessionCompaction(input: {
    ...
    const handoffMarkdown = [
      "SiriusEcoSystem session handoff:",
  ...
  async function executeRun(runId: string) {
    ...
    context.siriusEcoSystemWorkspace = {
      cwd: executionWorkspace.cwd,
      source: executionWorkspace.source,
      mode: executionWorkspaceMode,
    ...
    context.siriusEcoSystemWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.siriusEcoSystemRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.siriusEcoSystemRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.siriusEcoSystemSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.siriusEcoSystemSessionRotationReason = sessionCompaction.reason;
      context.siriusEcoSystemPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.siriusEcoSystemSessionHandoffMarkdown;
      delete context.siriusEcoSystemSessionRotationReason;
      delete context.siriusEcoSystemPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.siriusEcoSystemRuntimeServices = runtimeServices;
        context.siriusEcoSystemRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[siriusEcoSystem] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected SIRIUSLY_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.siriusEcoSystemRuntimeServices = combinedRuntimeServices;
        context.siriusEcoSystemRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[siriusEcoSystem] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
