...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@sirius-eco-system/db";
...
import type {
  ...
  PluginWorkspace,
  IssueComment,
} from "@sirius-eco-system/plugin-sdk";
 ...
 *
 * This implementation delegates to the core SiriusEcoSystem domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@sirius-eco-system/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
