 ...
 *    (`~/.siriusEcoSystem/plugins/`) and `node_modules` for packages matching
 *    the `sirius-eco-system-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published SiriusEcoSystem plugins.
 * Packages matching this pattern are considered SiriusEcoSystem plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "sirius-eco-system-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: SiriusEcoSystemPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.siriusEcoSystem/plugins/ local directory
  | "npm"               // npm packages matching sirius-eco-system-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: SiriusEcoSystemPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.siriusEcoSystem/plugins/
   */
  ...
  /**
   * Whether to discover installed npm packages matching the sirius-eco-system-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "sirius-eco-system-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: SiriusEcoSystemPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover SiriusEcoSystem plugins installed as npm packages in the current
   * Node.js environment matching the "sirius-eco-system-plugin-*" naming convention.
   *
   ...
   * the "siriusEcoSystemPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a SiriusEcoSystem plugin.
   * Throws if the package is a SiriusEcoSystem plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<SiriusEcoSystemPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: SiriusEcoSystemPluginManifestV1;
    newManifest: SiriusEcoSystemPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the SiriusEcoSystem plugin naming convention.
 * Accepts both the "sirius-eco-system-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @sirius-eco-system/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "siriusEcoSystemPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const siriusEcoSystemPlugin = pkgJson["siriusEcoSystemPlugin"];
  if (
    siriusEcoSystemPlugin !== null &&
    typeof siriusEcoSystemPlugin === "object" &&
    !Array.isArray(siriusEcoSystemPlugin)
  ) {
    const manifestRelPath = (siriusEcoSystemPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: SiriusEcoSystemPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumSiriusEcoSystemVersion;
}
 ...
 *   packageName: "sirius-eco-system-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: SiriusEcoSystemPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as SiriusEcoSystemPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a SiriusEcoSystem plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<SiriusEcoSystemPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a SiriusEcoSystem plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasSiriusEcoSystemPlugin = "siriusEcoSystemPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasSiriusEcoSystemPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<SiriusEcoSystemPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: SiriusEcoSystemPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @sirius-eco-system/shared exports). Run those workers through
