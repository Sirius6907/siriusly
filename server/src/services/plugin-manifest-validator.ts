 ...
 */
import { pluginManifestV1Schema } from "@sirius-eco-system/shared";
import type { SiriusEcoSystemPluginManifestV1 } from "@sirius-eco-system/shared";
import { PLUGIN_API_VERSION } from "@sirius-eco-system/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): SiriusEcoSystemPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as SiriusEcoSystemPluginManifestV1,
        };
      }
