...
import type { SiriusEcoSystemPluginManifestV1 } from "@sirius-eco-system/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __siriusEcoSystem_exports: module.exports,
      __siriusEcoSystem_module: module,
      __siriusEcoSystem_require: requireInSandbox,
      __siriusEcoSystem_filename: realPath,
      __siriusEcoSystem_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__siriusEcoSystem_exports, __siriusEcoSystem_module, __siriusEcoSystem_require, __siriusEcoSystem_filename, __siriusEcoSystem_dirname)`;
