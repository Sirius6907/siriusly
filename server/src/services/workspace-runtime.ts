...
import type { AdapterRuntimeServiceReport } from "@sirius-eco-system/adapter-utils";
import type { Db } from "@sirius-eco-system/db";
import { workspaceRuntimeServices } from "@sirius-eco-system/db";
...
function sanitizeBranchName(value: string): string {
  return value
    ...
    .replace(/-+/g, "-")
    .replace(/^[-/.]+|[-/.]+$/g, "")
    .slice(0, 120) || "sirius-eco-system-work";
}
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.SIRIUSLY_WORKSPACE_CWD = input.worktreePath;
  env.SIRIUSLY_WORKSPACE_PATH = input.worktreePath;
  env.SIRIUSLY_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.SIRIUSLY_WORKSPACE_BRANCH = input.branchName;
  env.SIRIUSLY_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.SIRIUSLY_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.SIRIUSLY_WORKSPACE_SOURCE = input.base.source;
  env.SIRIUSLY_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.SIRIUSLY_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.SIRIUSLY_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.SIRIUSLY_PROJECT_ID = input.base.projectId ?? "";
  env.SIRIUSLY_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.SIRIUSLY_AGENT_ID = input.agent.id;
  env.SIRIUSLY_AGENT_NAME = input.agent.name;
  env.SIRIUSLY_COMPANY_ID = input.agent.companyId;
  env.SIRIUSLY_ISSUE_ID = input.issue?.id ?? "";
  env.SIRIUSLY_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.SIRIUSLY_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.SIRIUSLY_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.SIRIUSLY_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.SIRIUSLY_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.SIRIUSLY_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.SIRIUSLY_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.SIRIUSLY_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.SIRIUSLY_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.SIRIUSLY_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.SIRIUSLY_PROJECT_ID = input.workspace.projectId ?? "";
  env.SIRIUSLY_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.SIRIUSLY_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".siriusEcoSystem", "worktrees");
