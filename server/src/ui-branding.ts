const FAVICON_BLOCK_START = "<!-- SIRIUSLY_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- SIRIUSLY_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- SIRIUSLY_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- SIRIUSLY_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.SIRIUSLY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.SIRIUSLY_WORKTREE_NAME) ?? nonEmpty(env.SIRIUSLY_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.SIRIUSLY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="sirius-eco-system-worktree-enabled" content="true" />',
    `<meta name="sirius-eco-system-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="sirius-eco-system-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="sirius-eco-system-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
