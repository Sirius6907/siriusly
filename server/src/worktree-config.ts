...
import type { SiriusEcoSystemConfig } from "@sirius-eco-system/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by SiriusEcoSystem worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.SIRIUSLY_IN_WORKTREE !== "true") return null;

  const configPath = resolveSiriusEcoSystemConfigPath(overrideConfigPath);
  const envPath = resolveSiriusEcoSystemEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.SIRIUSLY_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.SIRIUSLY_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.SIRIUSLY_HOME) ??
      nonEmpty(env.SIRIUSLY_WORKTREES_DIR) ??
      "~/.sirius-eco-system-worktrees",
  );
...
function writeConfigFile(configPath: string, config: SiriusEcoSystemConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".siriusEcoSystem", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as SiriusEcoSystemConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: SiriusEcoSystemConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: SiriusEcoSystemConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.SIRIUSLY_HOME = context.homeDir;
  process.env.SIRIUSLY_INSTANCE_ID = context.instanceId;
  process.env.SIRIUSLY_CONFIG = context.configPath;
  process.env.SIRIUSLY_CONTEXT = context.contextPath;
  process.env.SIRIUSLY_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SiriusEcoSystemConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    SIRIUSLY_HOME: context.homeDir,
    SIRIUSLY_INSTANCE_ID: context.instanceId,
    SIRIUSLY_CONFIG: context.configPath,
    SIRIUSLY_CONTEXT: context.contextPath,
    SIRIUSLY_IN_WORKTREE: "true",
    SIRIUSLY_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: SiriusEcoSystemConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as SiriusEcoSystemConfig;
  } catch {
    return;
  }
