...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const effectiveCompanyId = !isCreate ? props.agent.companyId : props.companyId;
  const { data: fetchedModels } = useQuery({
    queryKey: ["adapter-models", adapterType, effectiveCompanyId],
    queryFn: () => effectiveCompanyId ? agentsApi.adapterModels(effectiveCompanyId, adapterType) : Promise.resolve([]),
    enabled: !!effectiveCompanyId,
  });
  const models = fetchedModels ?? externalModels ?? [];

  // Section toggle state G�� advanced always starts collapsed
    ...
    <div className="relative">
      ...
      <div className={cn(isCreate ? "border-t border-border" : "border-b border-border")}>
        ...
        <div className="px-4 pb-3 space-y-3">
          ...
          {/* Advanced adapter section G�� collapsible in both modes */}
          {isLocal && (
            <CollapsibleSection
