...
type SiriusEcoSystemSprite = (typeof SIRIUSLY_SPRITES)[number];
...
function spriteSize(sprite: SiriusEcoSystemSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SIRIUSLY_SPRITES[(baseCol + baseRow) % SIRIUSLY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SIRIUSLY_SPRITES[Math.floor(Math.random() * SIRIUSLY_SPRITES.length)]!;
