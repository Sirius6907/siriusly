...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`sirius-eco-system-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="sirius-eco-system-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("sirius-eco-system-mermaid-status", error && "sirius-eco-system-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="sirius-eco-system-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `sirius-eco-system-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "sirius-eco-system-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "sirius-eco-system-markdown prose prose-sm max-w-none break-words overflow-hidden",
