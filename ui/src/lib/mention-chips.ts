...
import { parseAgentMentionHref, parseProjectMentionHref } from "@sirius-eco-system/shared";
...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--sirius-eco-system-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--sirius-eco-system-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("sirius-eco-system-mention-chip", `sirius-eco-system-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("sirius-eco-system-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "sirius-eco-system-mention-chip",
    "sirius-eco-system-mention-chip--agent",
    "sirius-eco-system-mention-chip--project",
    "sirius-eco-system-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--sirius-eco-system-mention-project-color");
  element.style.removeProperty("--sirius-eco-system-mention-icon-mask");
}
