...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("sirius-eco-system-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("sirius-eco-system-demo/COMPANY.md");
  ...
  it("reads a SiriusEcoSystem zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".siriusEcoSystem.yaml": "schema: siriusEcoSystem/v1\n",
      },
      "sirius-eco-system-demo",
    );
  ...
  it("reads standard DEFLATE zip archives created outside SiriusEcoSystem", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("sirius-eco-system-demo");
