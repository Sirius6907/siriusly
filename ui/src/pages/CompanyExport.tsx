...
/**
 * Filter .siriusEcoSystem.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterSiriusEcoSystemYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://siriusEcoSystem.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm sirius-eco-system company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [SiriusEcoSystem](https://siriusEcoSystem.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [SiriusEcoSystem](https://siriusEcoSystem.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .siriusEcoSystem.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .siriusEcoSystem.yaml
    const yamlPath = exportData.siriusEcoSystemExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterSiriusEcoSystemYaml(exportData.files[yamlPath], checkedFiles);
    }
