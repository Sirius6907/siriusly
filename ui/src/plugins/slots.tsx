 ...
 *
 * Plugin bundles are built with `external: ["@sirius-eco-system/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__siriusEcoSystemPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__siriusEcoSystemPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__siriusEcoSystemPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__siriusEcoSystemPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@sirius-eco-system/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@sirius-eco-system/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@sirius-eco-system/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@sirius-eco-system/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@sirius-eco-system/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__siriusEcoSystemPluginBridge__) {
 ...
 * self-register via `window.siriusEcoSystemPlugins.registerReactComponent()`.
 ...
 * of `@sirius-eco-system/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
